    ...
    if ret:
        ...
        if abs(l) <= 155 or abs(r) <= 155:
            if l == 0 or r == 0:
                if l < 0 or r < 0:
                    print('left')
                    message = can.Message(arbitration_id=0x11, is_extended_id=False,data=[0x4C])
                    bus.send(message, timeout=0.2)
                elif l > 0 or r > 0:
                    print('right')
                    message = can.Message(arbitration_id=0x11, is_extended_id=False,data=[0x52])
                    bus.send(message, timeout=0.2)
            elif abs(l - 15) > abs(r):
                print('right')
                message = can.Message(arbitration_id=0x11, is_extended_id=False,data=[0x52])
                ...
                print('go')
                message = can.Message(arbitration_id=0x11, is_extended_id=False,data=[0x47])
                bus.send(message, timeout=0.2)
        else:
            if l > 155 or r > 155:
                print('hard right')
